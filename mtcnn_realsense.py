...
rs_config.enable_stream(rs.stream.color, 960, 540, rs.format.bgr8, 30)#啟用彩色流，設置解析度為 960x540，像素格式為 bgr8（8 位 RGB 彩色），幀率為 30 幀/秒
...
try:
    while True:
        ...
        if nrof_faces > 0:
            ...
            det_arr = []
            img_size = np.asarray(img.shape)[0:2]#得到圖像的高度和寬度
            ...
            for i, det in enumerate(det_arr):#遍歷 det_arr 中的每個邊界框
                cv2.rectangle(img, (det[0],det[1]), (det[2],det[3]), color, 2)#在原始影像上繪製一個矩形
