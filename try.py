...
def calculate_distance_between_eyes(depth_frame, eye_points):
    # 從深度圖像中獲取相應的點的深度值
    depth = depth_frame.get_distance(eye_points[0], eye_points[1])
    # 假設攝影機和眼睛之間的距離，這個值可以根據實際情況調整
    camera_eye_distance = 0.5  # 單位：米
    # 計算兩眼之間的距離
    distance = math.sqrt(depth**2 + camera_eye_distance**2)
    return distance
    ...
    while True:
        ...
        if nrof_faces > 0:
            for i, det in enumerate(det_arr):
                # 提取眼睛的特徵點
                eye_points = points[i][:2]
                # 計算兩眼之間的距離
                distance = calculate_distance_between_eyes(depth_frame, eye_points)
                # 在畫面上顯示距離信息
                cv2.putText(color_image, "Distance: {:.2f} m".format(distance), (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
